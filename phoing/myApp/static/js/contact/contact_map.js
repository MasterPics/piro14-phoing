...
function displayContactMarkers(){
    ...
    for (let i = 0; i < contacts.length; i ++) {
        ...
        // 마커 위치
        let markerLoc = new kakao.maps.LatLng(contacts[i].lat, contacts[i].lon)
        
        // 마커를 생성합니다
        let marker = new kakao.maps.Marker({
            map: map, // 마커를 표시할 지도
            position: markerLoc, // 마커를 표시할 위치
            title : contacts[i].title, // 마커의 타이틀, 마커에 마우스를 올리면 타이틀이 표시됩니다
            image : markerImage // 마커 이미지 
        });

        let infowindow = new kakao.maps.InfoWindow({
            content: displayCustomInfoWindow(contacts[i]), // 인포윈도우에 표시할 내용
            zIndex: 3
        });
...
function displayCustomInfoWindow(contact) {
    let content = '<div class="info-window">' +
        '<div class="title"> Title: ' + contact.title + '</div>' +
        '<div class="pay"> Payment: ' + contact.pay + '</div>' + 
        '<div class="period"> Period: ' + contact.start_date + '-' + contact.end_date + '</div>' + 
        '<div class="address"> Address: ' + contact.address + '</div>' + 
        '<a href="/contact/detail/' + contact.pk + '/">자세히보기</a>' +  
    '</div>'
...
function giveDetailListner(index) {
    return function() {
        window.location.href = "/contact/detail/" + (index + 1) + "/";
    }
}
